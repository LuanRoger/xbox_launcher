...
from internals.element_picker import getGamesInfoInNewTab, getGameByIndex, getGridLenght, getTileGameImageUrl
...
grid_lenght: int = getGridLenght(driver)
games_list: List[XcloudGame] = []
while not done:
        for gameIndex in range(0, grid_lenght):
            game = getGameByIndex(driver, gameIndex)
        ...
        sleep(3)
        if(not done):
            grid_lenght = getGridLenght(driver)
